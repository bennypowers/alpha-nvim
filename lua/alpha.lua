...
local function layout(conf, state)
  ...
  local hl = {}
  local text = {}
  local computed_layout = type(conf.layout) == 'function' and conf.layout(conf, state) or conf.layout
  for _, el in pairs(computed_layout) do
...
local function enable_alpha(conf)
  ...
  local group_id = augroup('alpha_temp', { clear = true })
  ...
  if conf.opts then
    if if_nil(conf.opts.redraw_on_resize, true) then
      ...
      autocmd({ 'BufLeave', 'WinEnter', 'WinNew', 'WinClosed' }, {
        group = group_id,
        pattern = '*',
        callback = alpha.redraw,
      })
...
function alpha.setup(config)
  vim.validate {
    config = { config, "table" },
    layout = { config.layout, { "table", "function" } },
  }
  ...
  if current_config.opts.autostart then
    autocmd('VimEnter', {
      group = augroup('alpha_start', { clear = true }),
      pattern = '*',
      nested = true,
      callback = function()
        alpha.start(true)
